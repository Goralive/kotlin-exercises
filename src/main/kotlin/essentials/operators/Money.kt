...
data class Money(
    ...
    operator fun plus(other: Money): Money {
        if (currency != other.currency) {
            throw IllegalArgumentException("Can't process operation for money with currencies $currency and ${other.currency}")
        }
    ...
    operator fun unaryPlus() = Money(amount, currency)
    operator fun times(times: Int): Money {
        return Money(amount * times.toBigDecimal(), currency)
    }
